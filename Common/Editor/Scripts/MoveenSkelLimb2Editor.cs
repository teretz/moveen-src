...
namespace moveen.editor {
    ...
    public class MoveenSkelLimb2Editor : MoveenSkelWithBonesEditor {
        ...
        public override void OnInspectorGUI() {
            ...
            solveIk = GUILayout.Toggle(solveIk, "automatic tweak (editor only)");
            if (oldValue != solveIk) {
...
//            Gizmos.DrawLine(rotationJoint.basisAbs.v, rotationJoint.targetAbs.v);
        ...
        public override float evaluate() {
            MoveenSkelLimb2 skel = (MoveenSkelLimb2) this.skel;
            float result = base.evaluate();
            result += MyMath.pow(skel.r1 - skel.r2, 2)*0.001f;
            result += skel.r1 * 0.001f;
            result += skel.r2 * 0.001f;
            return result;
        }
    }
}